...
fn permit_pda(operator: &SolanaPubkey, permit_id: &[u8; 32]) -> (SolanaPubkey, u8) {
    SolanaPubkey::find_program_address(
        &[b"airspace_permit", operator.as_ref(), permit_id],
...
async fn test_submit_attestation_happy_path() {
    ...
    let (permit, _) = permit_pda(&operator, &permit_id);
