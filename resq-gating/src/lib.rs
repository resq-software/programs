...
pub mod resq_gating {
    ...
    pub fn submit_attestation(
        ...
        // are_coordinates_valid covers latitude/longitude (f64). Altitude is a
        // separate f32 and must also be finite before it is recorded: a signed
        // but malformed payload with NaN/±inf altitude would otherwise persist
        // verbatim and permanently advance the permit's waypoint sequence.
...
pub struct InitializePermit<'info> {
    #[account(
        ...
        // permit_id is GLOBALLY unique: the PDA is seeded by permit_id alone, not
        // (operator, permit_id). Including operator.key() previously let an
        // attacker init a second permit with a victim's permit_id + drone and
        // replay the victim's public on-chain attestations onto it (CWE-294).
        // With a global namespace an already-registered permit_id cannot be
        // re-initialized, so there is no shadow permit for a replay to land on.
        seeds = [b"airspace_permit".as_ref(), permit_id.as_ref()],
        bump
    )]
...
pub struct SubmitAttestation<'info> {
    #[account(
        mut,
        // Must match InitializePermit: permit_id alone (globally unique), no operator.
        seeds = [b"airspace_permit".as_ref(), permit.permit_id.as_ref()],
        bump = permit.bump
    )]
    pub permit: Account<'info, AirspacePermit>,
